  ...
  const sendTest = async (eventType: string) => {
    ...
    try {
      const res = await fetch('/api/notify', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
        ...
        data: { patient_name: 'Test Patient', doctor_name: 'Dr. Test', date: new Date().toLocaleDateString('en-IN'), time: '10:00 AM', centre_name: 'Health1', test_names: 'CBC, LFT', collection_point: 'Lab Reception', medicine_count: '3', pharmacy_counter: 'Counter 1', ipd_number: 'IPD-TEST-001', discharge_date: new Date().toLocaleDateString('en-IN'), follow_up_date: 'In 7 days' },
      }) });
