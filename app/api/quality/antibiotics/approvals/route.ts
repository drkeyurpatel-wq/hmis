...
export async function GET(request: NextRequest) {
  ...
  let q = db.from('quality_antibiotic_approvals').select('*').eq('centre_id', centreId).order('created_at', { ascending: false }).limit(100);
...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await db.from('quality_antibiotic_approvals').insert(body).select().single();
