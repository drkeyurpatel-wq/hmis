...
type ViewMode = 'patients' | 'tasks' | 'alerts' | 'handoff';
...
function NursingStationInner() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4">
      <AlertBanner alerts={alertSystem.alerts.filter(a => a.severity === 'emergency' || a.severity === 'critical')} onAcknowledge={(id) => alertSystem.acknowledge(id, staffId)} />
      ...
      <div className={`flex items-center justify-between ${alertSystem.counts.emergency > 0 || alertSystem.counts.critical > 0 ? 'mt-10' : ''}`}>
      ...
      <div className="flex gap-1">
        {(['patients', 'tasks', 'alerts', 'handoff'] as ViewMode[]).map(v =>
          <button key={v} onClick={() => setView(v)} className={`px-4 py-2 text-xs font-medium rounded-lg ${view === v ? 'bg-blue-600 text-white' : 'bg-white border'} ${v === 'alerts' && alertSystem.counts.total > 0 ? 'relative' : ''}`}>
            {v === 'patients' ? 'Patient Board' : v === 'tasks' ? 'Task Board' : v === 'alerts' ? 'Alerts' : 'Shift Handoff'}
            {v === 'alerts' && alertSystem.counts.total > 0 && <span className="ml-1 px-1.5 py-0.5 bg-red-500 text-white text-[9px] rounded-full font-bold">{alertSystem.counts.total}</span>}
          </button>
        )}
      </div>
      ...
      {view === 'alerts' && (
        <AlertPanel
          ...
          loading={alertSystem.loading}
          onAcknowledge={(id) => { alertSystem.acknowledge(id, staffId); flash('Alert acknowledged'); }}
          onResolve={(id, note) => { alertSystem.resolve(id, staffId, note); flash('Alert resolved'); }}
          onRefresh={alertSystem.load}
        />
      )}
