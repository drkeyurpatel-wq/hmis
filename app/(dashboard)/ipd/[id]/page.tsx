...
function IPDClinicalInner() {
  ...
  const patientAlerts = useAlerts(admission?.centre_id || '', admissionId);
  ...
  const tabs: [ClinicalTab, string, string][] = [
    ...
    ['nursing', 'Nursing', ''],
    ['alerts', 'Alerts', `${patientAlerts.counts.total}`],
    ['lab', 'Lab', ''],
    ['imaging', 'Imaging', ''],
  ...
  return (
    <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-xs text-gray-500 flex-wrap">
                ...
                {patientAlerts.counts.total > 0 && <button onClick={() => setTab('alerts')} className={`px-1.5 py-0.5 rounded font-bold text-[10px] animate-pulse ${patientAlerts.counts.emergency > 0 ? 'bg-red-600 text-white' : 'bg-amber-100 text-amber-700'}`}>{patientAlerts.counts.total} Alert{patientAlerts.counts.total !== 1 ? 's' : ''}</button>}
              </div>
      ...
      {tab === 'alerts' && <AlertPanel alerts={patientAlerts.alerts} loading={patientAlerts.loading} onAcknowledge={(id) => { patientAlerts.acknowledge(id, staffId); flash('Alert acknowledged'); }} onResolve={(id, note) => { patientAlerts.resolve(id, staffId, note); flash('Alert resolved'); }} onRefresh={patientAlerts.load} />}
