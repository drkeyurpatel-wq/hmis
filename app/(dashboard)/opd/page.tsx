...
const SC: Record<string, { label: string; badge: string; dot: string }> = {
  waiting: { label: 'Waiting', badge: 'inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-h1-yellow-light text-h1-yellow', dot: 'bg-h1-yellow' },
  checked_in: { label: 'Checked In', badge: 'inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-h1-teal-light text-h1-teal', dot: 'bg-h1-teal' },
  with_doctor: { label: 'With Doctor', badge: 'inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-h1-navy-light text-h1-navy', dot: 'bg-h1-navy' },
  completed: { label: 'Completed', badge: 'inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-green-50 text-h1-success', dot: 'bg-h1-success' },
  cancelled: { label: 'Cancelled', badge: 'inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-h1-red-light text-h1-red', dot: 'bg-h1-red' },
...
function OPDInner() {
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-4">
      {toast && <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-h1-navy text-white px-5 py-2.5 rounded-xl shadow-lg text-sm font-medium">{toast}</div>}
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowNew(true)} className="flex items-center gap-2 px-4 py-2.5 bg-h1-navy text-white text-sm rounded-xl font-semibold hover:bg-h1-navy/90"><Plus size={15} /> New OPD Visit</button>
      </div>

      <div className="grid grid-cols-7 gap-2">
        {[
          { l: 'Total', v: stats?.total || 0, c: 'text-gray-800' },
          { l: 'Waiting', v: visits.filter(v => v.status === 'waiting').length, c: 'text-h1-yellow' },
          { l: 'Checked In', v: visits.filter(v => v.status === 'checked_in').length, c: 'text-h1-teal' },
          { l: 'With Doctor', v: stats?.withDoctor || 0, c: 'text-h1-navy' },
          { l: 'Completed', v: stats?.completed || 0, c: 'text-h1-success' },
          { l: 'No Shows', v: visits.filter(v => v.status === 'no_show').length, c: 'text-red-600' },
          { l: 'Avg Wait', v: avgWait, c: 'text-h1-navy' },
        ].map(s => (
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex gap-1">{(['queue', 'doctor', 'list'] as ViewMode[]).map(v => (
          <button key={v} onClick={() => setView(v)} className={`px-3 py-2 text-xs font-medium rounded-xl capitalize ${view === v ? 'bg-h1-navy text-white' : 'bg-white text-gray-500 border'}`}>{v === 'doctor' ? 'By Doctor' : v === 'queue' ? 'Queue Board' : 'List'}</button>
        ))}</div>
        <div className="flex gap-1">{['active', 'all', 'completed'].map(f => (
          <button key={f} onClick={() => setFilter(f)} className={`px-2.5 py-1.5 text-[10px] font-medium rounded-lg ${filter === f ? 'bg-h1-navy-light text-h1-navy border border-h1-teal/30' : 'bg-gray-50 text-gray-500'}`}>{f === 'active' ? 'Active' : f === 'all' ? 'All' : 'Done'}</button>
        ))}</div>
                ...
                <div className="space-y-2 min-h-[300px]">
                  {items.map(v => (
                    <div key={v.id} onClick={() => v.status !== 'completed' && v.status !== 'cancelled' && v.status !== 'no_show' ? router.push(`/emr-v2?patient=${v.patient?.id}&visit=${v.id}`) : null} className={`bg-white rounded-xl border p-3 hover:shadow-md hover:-translate-y-0.5 transition-all group ${v.status !== 'completed' && v.status !== 'cancelled' ? 'cursor-pointer' : ''}`}>
                      <div className="flex items-center justify-between mb-1"><span className="text-xs font-bold text-h1-teal">T{v.tokenNumber}</span><span className="text-[9px] text-gray-400">{waitTime(v)}</span></div>
                      ...
                      <div className="text-[10px] text-h1-teal font-medium mt-1">Dr. {v.doctor?.name?.split(' ').pop()}</div>
                      {status !== 'completed' && (
                        <button onClick={(e: any) => { e.stopPropagation(); moveNext(v); }} className="mt-2 w-full flex items-center justify-center gap-1 px-2 py-1.5 bg-h1-navy-light text-h1-navy text-[10px] font-semibold rounded-lg hover:bg-h1-navy-light opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {doctorQueues.map(dq => (
            <div key={dq.doctor.id || dq.doctor.name} className="bg-white rounded-2xl border overflow-hidden">
              <div className="px-4 py-3 border-b bg-gray-50/50 flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-h1-teal-light flex items-center justify-center"><Stethoscope size={16} className="text-h1-navy" /></div>
                ...
                <span className="text-lg font-black text-h1-teal">{dq.visits.length}</span>
              </div>
                ...
                {dq.visits.map((v, i) => {
                  ...
                  return (
                    <div key={v.id} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-50">
                      ...
                      {v.status !== 'completed' && <button onClick={() => moveNext(v)} className="p-1 text-h1-teal hover:bg-h1-navy-light rounded"><ChevronRight size={14} /></button>}
                    </div>
                  );
                })}
              </div>
      ...
      {showNew && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowNew(false)}>
          <div className="bg-white rounded-2xl w-full max-w-lg p-6 space-y-4" onClick={e => e.stopPropagation()}>
            ...
            {selPat ? (
              <div className="flex items-center gap-3 bg-h1-navy-light rounded-xl p-3 border border-h1-teal/30">
                <div className="w-10 h-10 rounded-full bg-h1-teal-light flex items-center justify-center"><span className="font-bold text-h1-navy">{selPat.first_name?.[0]}{selPat.last_name?.[0]}</span></div>
              ...
              <div className="space-y-2">
                <div className="relative"><input value={patSearch} onChange={e => setPatSearch(e.target.value)} className="w-full px-3 py-2.5 border rounded-xl text-sm" placeholder="Search patient by name, UHID, phone..." />
                  {patResults.length > 0 && <div className="absolute z-10 mt-1 w-full bg-white border rounded-xl shadow-xl max-h-48 overflow-y-auto">{patResults.map(p => <button key={p.id} onClick={() => { setSelPat(p); setPatSearch(''); setPatResults([]); }} className="w-full text-left px-3 py-2.5 hover:bg-h1-navy-light border-b text-xs"><span className="font-medium">{p.first_name} {p.last_name}</span> · {p.uhid} · {p.phone_primary}</button>)}</div>}</div>
                <button onClick={() => setShowQuickReg(true)} className="flex items-center gap-1.5 text-xs text-h1-teal font-medium"><UserPlus size={12} /> Quick Register</button>
              </div>
            )}
            {showQuickReg && !selPat && (
              <div className="bg-gray-50 rounded-xl p-4 space-y-3 border"><h3 className="text-sm font-bold">Quick Registration</h3>
                ...
                <div className="flex gap-1">{['male', 'female', 'other'].map(g => <button key={g} onClick={() => setRegForm(f => ({ ...f, gender: g }))} className={`flex-1 py-2 rounded-lg text-xs font-medium capitalize ${regForm.gender === g ? 'bg-h1-navy text-white' : 'bg-white text-gray-500 border'}`}>{g}</button>)}</div>
                <button onClick={handleQuickReg} disabled={!regForm.first_name || !regForm.phone} className="w-full py-2 bg-h1-navy text-white text-sm rounded-xl font-semibold disabled:opacity-40">Register & Select</button>
              </div>
            )}
            ...
            <button onClick={handleCreate} disabled={!selPat || !selDoctor || creating} className="w-full py-3 bg-h1-navy text-white text-sm rounded-xl font-semibold disabled:opacity-40 hover:bg-h1-navy/90">{creating ? 'Creating...' : 'Register & Generate Token'}</button>
          </div>
        </div>
      )}
    </div>
