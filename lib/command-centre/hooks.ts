...
export function useCommandCentre() {
  ...
  const load = useCallback(async () => {
    ...
    const [bedResult, opsResult, revResult, insResult] = await Promise.allSettled([
      ...
      sb().rpc('get_bed_census').then(({ data, error }: any) => {
      ...
      sb().rpc('get_daily_ops_summary', { p_date: today }).then(({ data, error }: any) => {
        if (error) throw error;
      ...
      sb().rpc('get_revenue_summary', { p_date: today }).then(({ data, error }: any) => {
        if (error) throw error;
        return { type: 'rpc', data };
      }).catch(async () => {
      ...
      sb().rpc('get_insurance_pipeline').then(({ data, error }: any) => {
        if (error) throw error;
        return { type: 'rpc', data };
      }).catch(async () => {
