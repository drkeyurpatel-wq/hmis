...
// Per-department lab report printing with Health1 branding
...
<div style="display:flex;justify-content:space-between;align-items:center;border-bottom:3px solid #1e40af;padding-bottom:10px;margin-bottom:12px">
  <div>
    <div style="font-size:18px;font-weight:700;color:#1e40af">${centreName || 'Health1 Super Speciality Hospital'}</div>
...
<div style="margin-top:12px;font-size:7px;color:#aaa;text-align:center">
  This is a computer-generated report. ${reportedAt ? 'Reported: ' + reportedAt : ''}<br/>Health1 Super Speciality Hospital — Quality Healthcare for All
</div>`;
...
export function sendLabReportWhatsApp(phone: string, data: {
  ...
  const message = encodeURIComponent(
    ...
    `_For queries, call Health1 Lab: 079-XXXXXXXX_`
  );
