...
export function useEMR(): EMRState {
  ...
  useEffect(() => {
    if (patient) {
      cachePatient(patient).catch((_e: unknown) => { /* lib: non-blocking */ });
    }
  }, [patient]);
  ...
  useEffect(() => {
    if (pastEncounters.length > 0 && selectedPatientId) {
      cacheEncounterBatch(pastEncounters.map(e => ({ ...e, patientId: selectedPatientId }))).catch((_e: unknown) => { /* lib: non-blocking */ });
    }
    ...
    if (isOnline()) {
      try {
        ...
        if (activeEncounterId) {
          deleteDraft(activeEncounterId).catch((_e: unknown) => { /* lib: non-blocking */ });
        }
